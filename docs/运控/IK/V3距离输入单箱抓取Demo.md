...
**姿态合同：**`updown=-0.3m`，左臂 `J1～J7=[155,-105,20,90,-90,-40,0]°`，右臂为 `[-155,-105,-20,90,90,40,0]°`，头部关节为0。直接读取 SRDF `whole_body/home`，不复制一套角度到规划器。卸货姿态另以 `unloading` 命名：左臂 `[130,-105,-180,20,-90,-30,0]°`，右臂 `[-130,-105,180,20,90,30,0]°`；默认远端流程后置释放，显式 `loaded_home` 才携箱回 Home。
...
- 初态为文档/SRDF Home；非选中臂关节及头部固定。`loaded_home` 策略最终回到 `updown=-0.3m` 的 Home；默认 `rear_release` 保留后置释放末态。不搜索底盘联合路径。
